...
namespace Application.BusinessLogicLayer.Modules.RecipeModule.Commands
{
    ...
    {
        private readonly IAddAndUpdateRecipeService _addAndUpdateRecipeService;

        public UpdateRecipeCommandHandler(RecipeBookDbContext context, IAddAndUpdateRecipeService addAndUpdateRecipeService) : base(context)
        {
            _addAndUpdateRecipeService = addAndUpdateRecipeService ?? throw new ArgumentNullException(nameof(addAndUpdateRecipeService));
        }
        ...
        protected override async Task<Result> Handler(UpdateRecipeCommand request, CancellationToken cancellationToken)
        {
            ...
            recipe.RecipeIngredients = await _addAndUpdateRecipeService.InitialNewRecipeIngredients(new InitialNewRecipeIngredientsDto(request.Ingredients, cancellationToken));
